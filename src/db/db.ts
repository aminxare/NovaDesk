...
export class NovaDeskDatabase extends Dexie {
  files!: Table<DBFileItem, string>;
  settings!: Table<DBSetting, string>;

  constructor() {
    super('NovaDeskDatabase');
...
export const db = new NovaDeskDatabase();

export async function initializeDefaultFiles() {
  try {
    ...
    console.error('Failed to initialize default files in IndexedDB:', err);
  }
}
